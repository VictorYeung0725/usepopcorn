...
export default function App() {
  ...
  //tell the browser that the movie data is loading
  const [isLoading, setIsLoading] = useState(false);
  //set error message
  const [error, setError] = useState('');
  //catch user input state
  const [query, setQuery] = useState('');
  //use unique id to pick a movie
  const [selectedId, setSelectedID] = useState(null);

  //test for QueryInput
  // const tempQuery = 'interstellasdasdasd';
  ...
  function handleAddWatched(movie) {
    ...
    // in localStorage we can only store key value paired , by using stinrg
